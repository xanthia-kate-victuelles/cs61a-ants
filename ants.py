...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        self.name = name
        self.exit = exit
        self.bees = []        # A list of Bees   self.place = instance = self >>>>>> self.place.bees = self.bees
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                #Need to allow both container and non container ant
                if self.ant.can_contain(insect): # ant currently occuputing can
                    self.ant.contain_ant(insect) #adds the insect, assumung it is the Bodyguard Ant
                elif insect.can_contain(self.ant):     #means that the insect is
                    non_container_ant=self.ant
                    self.ant=insect # Place.ant becomes the insect
                    self.ant.contain_ant(non_container_ant)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min_range = 0, max_range = float('inf')):
        ...
        while ant_place.name != beehive.name:
            if random_or_none(ant_place.bees) and min_range <= count <= max_range:
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        return ThrowerAnt.nearest_bee(self, beehive, self.min_range, self.max_range)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0: #if ant dies, double damage
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        if self.time_to_digest - self.digesting < 0:
            self.digesting = 0
...
class BodyguardAnt(Ant):
    ...
    def __init__(self, armor=2):
        Ant.__init__(self, armor)
        self.contained_ant = None  # The Ant hidden in this bodyguard (container)
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 9
        #can only contain min of two ants
        "*** YOUR CODE HERE ***"
    ...
    def action(self, colony):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant is not None: #if there is an ant, should alow to do action
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self)
        if self.queen_instance==0 and QueenAnt.queen_instance==0:
            self.impostor=False
            QueenAnt.queen_instance+=1
        "*** YOUR CODE HERE ***"
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            ScubaThrower.action(self,colony) #to perform action to colony
            current_place= self.place
            while current_place.exit: #reaching the end
                curr_ant= current_place.exit.ant #ant inside the current place
                if curr_ant:
                    if curr_ant not in self.buffed_ants: #if an ant is an ant and it is not in  the buffed ants
                        curr_ant.damage*=2
                        self.buffed_ants.append(curr_ant)
                    if isinstance(curr_ant, BodyguardAnt)and curr_ant.is_container:
                        if curr_ant.contained_ant and curr_ant.contained_ant not in self.buffed_ants: #IF THE BODY GUARD CONTAINS AN ANT
                            curr_ant.contained_ant.damage*=2
                            self.buffed_ants.append(curr_ant.contained_ant)
                current_place=current_place.exit

            # if p= place, p to be place on place on exit, or p.exit
            # p is not none
            #if there is an ant in the place, then double damage and save
            # if you get an ant that is not buffed, then add the buffed list after
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and target.already_scared == False:
            apply_effect(make_scare, target, 2)
            target.already_scared = True
            target.is_scared = False
        # END Problem EC
